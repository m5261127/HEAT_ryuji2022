        ...
        for ( int z=0; z < filelist4.size(); z++ ){
            ...
            if (ext == "fit" )
            {
                ...
                try{
                    ...
                    int counter=0;
                    for(int i=0; i<ui->anotherView->Height; i++){
                        for(int j=0; j<ui->anotherView->Width; j++){
                            DNtmp1=contents[counter];
                            fitsave+=DNtmp1;
                            counter++;
                        }
                    }
                    fitsave=fitsave/(ui->anotherView->Height*ui->anotherView->Width);
                }
    ...
    if (xmin < 0 || xmax > 327 || ymin < 0 || ymax > 247 || ymin > ymax ||
        ...
        for(i=ymin;i<ymax+1;i++){
            for(j=xmin;j<xmax+1;j++){
                ui->anotherView->calibrationLutImage(sl[i][j],in[i][j],i,j);
                p.setLabelText("Calibration Process \n" + QString::number(count) +
